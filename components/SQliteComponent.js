...
const insertIntoTable = async (db, tableName, data) => {
  ...
  try {
    await db.runAsync(query, ...values); // Correção: Simplificando a inserção
    console.log(`Registro inserido na tabela ${tableName}`);
  } catch (error) {
    console.error(`Erro ao inserir o registro na tabela '${tableName}':`, error);
    throw error;
  }
};
...
const insertMultipleIntoTables = async (databases, tablesData) => {
  
  for (const [dbName, dbTables] of tablesData) {
    const db = databases[dbName]; // Seleciona o banco de dados

    for (const [tableName, formDataArray] of dbTables) {
      for (const formData of formDataArray) {
        // Converte o FormData diretamente em um objeto de chaves e valores
        const data = {};
        ...
        try {
          // Usando a função já existente para inserção de dados
          await insertIntoTable(db, tableName, data);
          console.log(
            `Registro inserido no banco "${dbName}", tabela "${tableName}":`,
            data
          );
        } catch (error) {
          console.error(
            `Erro ao inserir no banco "${dbName}", tabela "${tableName}":`,
            error
          );
        }
      }
...
const manageTableAndInsertData = async (databaseName, tablesWithData) => {
  for (const table of tablesWithData) {
    const { tableName, columns, data } = table;

    if (!tableName || !Array.isArray(columns) || columns.length === 0) {
      throw new Error('Cada tabela deve ter um nome e uma lista de colunas válida.');
    }

    try {
      // Cria ou atualiza a tabela
      await createTable(databaseName, tableName, columns);

      // Insere dados, se fornecidos
      if (data && Array.isArray(data)) {
        for (const record of data) {
          await insertIntoTable(await openDatabase(databaseName), tableName, record);
        }
      }

      console.log(`Tabela '${tableName}' gerenciada com sucesso.`);
    } catch (error) {
      console.error(`Erro ao gerenciar a tabela '${tableName}':`, error);
      throw error;
    }
  }
};
...
export default {
  ...
  getTableSchema,
  obterForKeyInfo,
  insertMultipleIntoTables,
  insertIntoTable,
  manageTableAndInsertData,
};